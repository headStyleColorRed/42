...
int		get_next_line(int fd, char **line)
{
	...
	if ((state_keeper[fd] = ft_line_reader(fd, state_keeper[fd])) == 0)
		return (-1);
	if ((tmp = ft_strchr(state_keeper[fd], '\n')))
	{
		i = tmp - state_keeper[fd];
		tmp = ft_strdup(state_keeper[fd] + i + 1);
		if ((*line = ft_strndup(state_keeper[fd], i)) == 0)
			return (-1);
		free(state_keeper[fd]);
		state_keeper[fd] = ft_strdup(tmp);
		free(tmp);
		return (1);
	}
	else
	{
		if ((*line = ft_strdup(state_keeper[fd])) == 0)
			return (-1);
		free(state_keeper[fd]);
		state_keeper[fd] = NULL;
		return (0);
	}
}
